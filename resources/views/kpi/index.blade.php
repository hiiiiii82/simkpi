...
@section('page-title','Indikator KPI UP3 Surakarta')
@section('content')
...
<div class="card mb-4">
  <div class="card-header" style="border-left:4px solid {{ $kat->warna }}">
    <span style="font-weight:700;color:{{ $kat->warna }}">{{ $namaKat }}</span>
    <span style="font-size:11px;color:#64748B;margin-left:8px">Total bobot: <strong>{{ $inds->sum('bobot') }}%</strong></span>
  </div>
  <div class="table-responsive"><table class="table mb-0">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td style="font-size:12.5px;font-weight:500;max-width:260px">{{ $ind->nama }}</td>
        <td><span style="background:#EFF6FF;color:#1E40AF;padding:1px 7px;border-radius:8px;font-size:10px">{{ $ind->satuan }}</span></td>
        <td class="text-end" style="font-weight:600;font-size:12.5px">{{ number_format($ind->target,4) }}</td>
        <td class="text-center"><span class="badge bg-primary" style="font-size:11px">{{ $ind->bobot }}%</span></td>
        <td class="text-center" style="font-size:12px;font-weight:600;color:{{ $ind->arah=='naik'?'#059669':'#DC2626' }}">{{ $ind->arah=='naik'?'⬆ Naik':'⬇ Turun' }}</td>
        <td class="text-center">
          <button class="btn btn-sm btn-outline-primary me-1" data-bs-toggle="modal" data-bs-target="#mdlEdit"
            data-id="{{ $ind->id }}" data-nama="{{ $ind->nama }}" data-satuan="{{ $ind->satuan }}" data-target="{{ $ind->target }}" data-bobot="{{ $ind->bobot }}" data-arah="{{ $ind->arah }}" data-periode="{{ $ind->periode }}">
            <i class="bi bi-pencil"></i>
          </button>
          <form action="{{ route('kpi.destroy',$ind->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Hapus indikator ini?')">@csrf @method('DELETE')
...
<div class="modal fade" id="mdlTambah" tabindex="-1"><div class="modal-dialog modal-lg"><div class="modal-content" style="border-radius:13px;border:none">
  ...
  <form action="{{ route('kpi.store') }}" method="POST">@csrf
    <div class="modal-body p-4">
      <div class="row g-3">
        <div class="col-md-6"><label class="form-label fw-bold" style="font-size:11px">KATEGORI</label><select name="kategori_id" class="form-select" required><option value="">-- Pilih --</option>@foreach($kategoris as $k)<option value="{{ $k->id }}">{{ $k->nama }}</option>@endforeach</select></div>
